...
# 远程部署建议通过 --tls-cert/--tls-key 或反向代理启用 HTTPS
...
鉴权时拒绝启动，避免把本地学习数据和 LLM 接口裸露到网络。远程访问建议通过
`--tls-cert/--tls-key` 或反向代理启用 HTTPS。浏览器登录后会把 key 保存在本机
`localStorage`，后续 API 请求使用 `Authorization: Bearer <key>`。
